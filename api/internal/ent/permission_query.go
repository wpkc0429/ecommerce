...
func (_q *PermissionQuery) QueryUserPermissions() *UserPermissionQuery {
...
func (_q *PermissionQuery) WithUserPermissions(opts ...func(*UserPermissionQuery)) *PermissionQuery {
	query := (&UserPermissionClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *PermissionQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Permission, error) {
	...
	if query := _q.withUserPermissions; query != nil {
		if err := _q.loadUserPermissions(ctx, query, nodes,
			func(n *Permission) { n.Edges.UserPermissions = []*UserPermission{} },
			func(n *Permission, e *UserPermission) { n.Edges.UserPermissions = append(n.Edges.UserPermissions, e) }); err != nil {
			return nil, err
		}
	}
...
func (_q *PermissionQuery) loadUserPermissions(ctx context.Context, query *UserPermissionQuery, nodes []*Permission, init func(*Permission), assign func(*Permission, *UserPermission)) error {
	...
	query.Where(predicate.UserPermission(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(permission.UserPermissionsColumn), fks...))
	}))
