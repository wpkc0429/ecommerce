...
func (_q *CategoryQuery) QueryShop() *ShopQuery {
...
func (_q *CategoryQuery) QueryProductCategories() *ProductCategoryQuery {
...
func (_q *CategoryQuery) WithShop(opts ...func(*ShopQuery)) *CategoryQuery {
	query := (&ShopClient{config: _q.config}).Query()
...
func (_q *CategoryQuery) WithProductCategories(opts ...func(*ProductCategoryQuery)) *CategoryQuery {
	query := (&ProductCategoryClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *CategoryQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Category, error) {
	...
	if query := _q.withShop; query != nil {
		if err := _q.loadShop(ctx, query, nodes, nil,
			func(n *Category, e *Shop) { n.Edges.Shop = e }); err != nil {
			return nil, err
		}
	}
	...
	if query := _q.withProductCategories; query != nil {
		if err := _q.loadProductCategories(ctx, query, nodes,
			func(n *Category) { n.Edges.ProductCategories = []*ProductCategory{} },
			func(n *Category, e *ProductCategory) {
...
func (_q *CategoryQuery) loadShop(ctx context.Context, query *ShopQuery, nodes []*Category, init func(*Category), assign func(*Category, *Shop)) error {
...
func (_q *CategoryQuery) loadProductCategories(ctx context.Context, query *ProductCategoryQuery, nodes []*Category, init func(*Category), assign func(*Category, *ProductCategory)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int]*Category)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(productcategory.FieldCategoryID)
	}
	query.Where(predicate.ProductCategory(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(category.ProductCategoriesColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		...
		node, ok := nodeids[fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "category_id" returned %v for node %v`, fk, n)
		}
		assign(node, n)
	}
	return nil
...
func (_q *CategoryQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if fields := _q.ctx.Fields; len(fields) > 0 {
		...
		if _q.withShop != nil {
			_spec.Node.AddColumnOnce(category.FieldShopID)
		}
