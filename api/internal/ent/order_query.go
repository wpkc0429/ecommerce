...
func (_q *OrderQuery) QueryShop() *ShopQuery {
...
func (_q *OrderQuery) QueryMember() *MemberQuery {
...
func (_q *OrderQuery) WithShop(opts ...func(*ShopQuery)) *OrderQuery {
	query := (&ShopClient{config: _q.config}).Query()
...
func (_q *OrderQuery) WithMember(opts ...func(*MemberQuery)) *OrderQuery {
	query := (&MemberClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *OrderQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Order, error) {
	...
	if query := _q.withShop; query != nil {
		if err := _q.loadShop(ctx, query, nodes, nil,
			func(n *Order, e *Shop) { n.Edges.Shop = e }); err != nil {
			return nil, err
		}
	}
	if query := _q.withMember; query != nil {
		if err := _q.loadMember(ctx, query, nodes, nil,
			func(n *Order, e *Member) { n.Edges.Member = e }); err != nil {
			return nil, err
		}
	}
...
func (_q *OrderQuery) loadShop(ctx context.Context, query *ShopQuery, nodes []*Order, init func(*Order), assign func(*Order, *Shop)) error {
...
func (_q *OrderQuery) loadMember(ctx context.Context, query *MemberQuery, nodes []*Order, init func(*Order), assign func(*Order, *Member)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Order)
	for i := range nodes {
		fk := nodes[i].MemberID
...
func (_q *OrderQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if fields := _q.ctx.Fields; len(fields) > 0 {
		...
		if _q.withShop != nil {
			_spec.Node.AddColumnOnce(order.FieldShopID)
		}
