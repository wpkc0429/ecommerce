...
type UserQuery struct {
	...
	withShopUsers       *ShopUserQuery
	withRoleUsers       *RoleUserQuery
...
// QueryShopUsers chains the current query on the "shop_users" edge.
func (_q *UserQuery) QueryShopUsers() *ShopUserQuery {
	query := (&ShopUserClient{config: _q.config}).Query()
...
func (_q *UserQuery) QueryRoleUsers() *RoleUserQuery {
...
func (_q *UserQuery) Clone() *UserQuery {
	...
	return &UserQuery{
		...
		withShopUsers:       _q.withShopUsers.Clone(),
		withRoleUsers:       _q.withRoleUsers.Clone(),
...
// WithShopUsers tells the query-builder to eager-load the nodes that are connected to
// the "shop_users" edge. The optional arguments are used to configure the query builder of the edge.
func (_q *UserQuery) WithShopUsers(opts ...func(*ShopUserQuery)) *UserQuery {
	query := (&ShopUserClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	_q.withShopUsers = query
	return _q
}
...
func (_q *UserQuery) WithRoleUsers(opts ...func(*RoleUserQuery)) *UserQuery {
	query := (&RoleUserClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	_q.withRoleUsers = query
	return _q
}
...
func (_q *UserQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*User, error) {
	var (
		...
		_spec       = _q.querySpec()
		loadedTypes = [4]bool{
			_q.withShopUsers != nil,
			_q.withRoleUsers != nil,
	...
	if query := _q.withShopUsers; query != nil {
		if err := _q.loadShopUsers(ctx, query, nodes,
			func(n *User) { n.Edges.ShopUsers = []*ShopUser{} },
			func(n *User, e *ShopUser) { n.Edges.ShopUsers = append(n.Edges.ShopUsers, e) }); err != nil {
			return nil, err
		}
	}
	if query := _q.withRoleUsers; query != nil {
		if err := _q.loadRoleUsers(ctx, query, nodes,
			func(n *User) { n.Edges.RoleUsers = []*RoleUser{} },
			func(n *User, e *RoleUser) { n.Edges.RoleUsers = append(n.Edges.RoleUsers, e) }); err != nil {
			return nil, err
		}
	}
...
func (_q *UserQuery) loadShopUsers(ctx context.Context, query *ShopUserQuery, nodes []*User, init func(*User), assign func(*User, *ShopUser)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(shopuser.FieldUserID)
	}
	query.Where(predicate.ShopUser(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(user.ShopUsersColumn), fks...))
	}))
...
func (_q *UserQuery) loadRoleUsers(ctx context.Context, query *RoleUserQuery, nodes []*User, init func(*User), assign func(*User, *RoleUser)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int]*User)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	query.Where(predicate.RoleUser(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(user.RoleUsersColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
...
func (_q *UserQuery) loadRefreshTokens(ctx context.Context, query *UserRefreshTokenQuery, nodes []*User, init func(*User), assign func(*User, *UserRefreshToken)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(userrefreshtoken.FieldUserID)
	}
