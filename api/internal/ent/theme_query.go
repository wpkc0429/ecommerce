...
func (_q *ThemeQuery) QueryPages() *ThemePageQuery {
	query := (&ThemePageClient{config: _q.config}).Query()
...
func (_q *ThemeQuery) QueryShops() *ShopQuery {
...
func (_q *ThemeQuery) WithPages(opts ...func(*ThemePageQuery)) *ThemeQuery {
	query := (&ThemePageClient{config: _q.config}).Query()
...
func (_q *ThemeQuery) WithShops(opts ...func(*ShopQuery)) *ThemeQuery {
	query := (&ShopClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *ThemeQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Theme, error) {
	...
	if query := _q.withPages; query != nil {
		if err := _q.loadPages(ctx, query, nodes,
			func(n *Theme) { n.Edges.Pages = []*ThemePage{} },
			func(n *Theme, e *ThemePage) { n.Edges.Pages = append(n.Edges.Pages, e) }); err != nil {
			return nil, err
		}
	}
	if query := _q.withShops; query != nil {
		if err := _q.loadShops(ctx, query, nodes,
			func(n *Theme) { n.Edges.Shops = []*Shop{} },
			func(n *Theme, e *Shop) { n.Edges.Shops = append(n.Edges.Shops, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *ThemeQuery) loadPages(ctx context.Context, query *ThemePageQuery, nodes []*Theme, init func(*Theme), assign func(*Theme, *ThemePage)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int]*Theme)
	for i := range nodes {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(themepage.FieldThemeID)
	}
	query.Where(predicate.ThemePage(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(theme.PagesColumn), fks...))
	}))
	...
	for _, n := range neighbors {
		...
		node, ok := nodeids[fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "theme_id" returned %v for node %v`, fk, n.ID)
		}
		assign(node, n)
	}
	return nil
}
func (_q *ThemeQuery) loadShops(ctx context.Context, query *ShopQuery, nodes []*Theme, init func(*Theme), assign func(*Theme, *Shop)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int]*Theme)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(shop.FieldThemeID)
	}
	query.Where(predicate.Shop(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(theme.ShopsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.ThemeID
		if fk == nil {
			return fmt.Errorf(`foreign-key "theme_id" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "theme_id" returned %v for node %v`, *fk, n.ID)
