...
func (_q *SiteQuery) QueryShops() *ShopQuery {
...
func (_q *SiteQuery) QuerySiteMappings() *SiteShopQuery {
...
func (_q *SiteQuery) WithShops(opts ...func(*ShopQuery)) *SiteQuery {
	query := (&ShopClient{config: _q.config}).Query()
...
func (_q *SiteQuery) WithSiteMappings(opts ...func(*SiteShopQuery)) *SiteQuery {
	query := (&SiteShopClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *SiteQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Site, error) {
	...
	if query := _q.withShops; query != nil {
		if err := _q.loadShops(ctx, query, nodes,
			func(n *Site) { n.Edges.Shops = []*Shop{} },
			func(n *Site, e *Shop) { n.Edges.Shops = append(n.Edges.Shops, e) }); err != nil {
			return nil, err
		}
	}
	if query := _q.withSiteMappings; query != nil {
		if err := _q.loadSiteMappings(ctx, query, nodes,
			func(n *Site) { n.Edges.SiteMappings = []*SiteShop{} },
			func(n *Site, e *SiteShop) { n.Edges.SiteMappings = append(n.Edges.SiteMappings, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *SiteQuery) loadShops(ctx context.Context, query *ShopQuery, nodes []*Site, init func(*Site), assign func(*Site, *Shop)) error {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(site.ShopsTable)
		s.Join(joinT).On(s.C(shop.FieldID), joinT.C(site.ShopsPrimaryKey[1]))
		s.Where(sql.InValues(joinT.C(site.ShopsPrimaryKey[0]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(site.ShopsPrimaryKey[0]))
	...
	neighbors, err := withInterceptors[[]*Shop](ctx, query, qr, query.inters)
	if err != nil {
		return err
	}
	for _, n := range neighbors {
		nodes, ok := nids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected "shops" node returned %v`, n.ID)
		}
		for kn := range nodes {
			assign(kn, n)
		}
...
func (_q *SiteQuery) loadSiteMappings(ctx context.Context, query *SiteShopQuery, nodes []*Site, init func(*Site), assign func(*Site, *SiteShop)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(siteshop.FieldSiteID)
	}
	query.Where(predicate.SiteShop(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(site.SiteMappingsColumn), fks...))
	}))
	...
	for _, n := range neighbors {
		...
		node, ok := nodeids[fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "site_id" returned %v for node %v`, fk, n)
		}
		assign(node, n)
	}
	return nil
